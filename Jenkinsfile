pipeline {
  ...
  environment {
    ...
    CONTAINER_NAME = 'consort-payment-service'
    DOCKER_NETWORK = 'consort-infra_consort-network'
    LOGS_DIR       = '/app/logs/payment-service'
    ...
    stage('Load .env') {
      steps {
        script {
          ...
          env.SPRING_KAFKA_BOOTSTRAP_SERVERS = rd('SPRING_KAFKA_BOOTSTRAP_SERVERS') ?: 'consort-kafka:9092'
        }
      }
    }
      ...
      steps {
        sh '''
          ...
          docker run -d \
            ...
            -p ${APP_PORT}:${APP_PORT} \
            -v ${LOGS_DIR}:/app/logs/payment-service \
            --env-file ${ENV_FILE} \
            -e TZ=Asia/Tashkent \
